...
However, $\mathbb{D}$ is not compact when $k > 0$ an
...
The parameters $b$ and $c$ are restricted such that the set $\mathbb{D}_k$ is bounded in $\lp{q}(\Omega)$.
...
\begin{proposition}
    ...
    the choice of $c_1, c_2$ in $\mathbb{D}_1$.
    \begin{equation}
...
\begin{proof}
    ...
    Since B is compact set on $\lp{2}(\Omega)$, it follows from Lemma
    \ref{lemma:compact_set_integral_representation} that there is Borel measure
    $\mu$ on $G$ such that 
    \begin{equation}
        f = \int_G \inclusionMap{G}{\lp{2}(\Omega)} \,d\mu
    \end{equation}
    ...
    \begin{align*}
        \normVar{f}{G}
        &= \inf_{\mu} \Big\{ 
            \norm{\mu}_{G}: f = \int_G \inclusionMap{G}{\lp{2}(\Omega)} \,d\mu
        \Big\} \\
        &= \inf_{\rho} \Big\{
