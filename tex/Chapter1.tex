...
In the previous chapter, we have introduced the basic concepts about the
two-layer neural networks (\gls{2nn}). Section \ref{sec:preliminary} introduces
the approximation theory concerning $n$-term dictionary in a Hilbert space. In
section \ref{sec:uat}, we will state the result of the universal approximation
theorem. Later in section \ref{sec:spectral_norm} and
\ref{sec:different_barron_spaces}, we formally introduce the function spaces
associated with 2NN: the \textit{Fourier-analytic Barron spaces} and
\textit{infinite-width Barron spaces}. The norms associated with these sapces
(\textit{spectral norm}, \textit{Barron norm}) are examined in Section
\ref{sec:different_barron_spaces}. An error rate of $\bigO(n^{-1/2})$ for
functions in these spaces are given in Section \ref{sec:spectral_norm} and
\ref{sec:barron_norm}. The aim of this chapter is to provide a concise summary
of various well-known results concerning the approximation properties of 2NN and
we recommend work by
...
target function $f \in \mcal{H}$, to choose both a dictionary $\mathbb{D}$ and
an $n$-term approximation to $f$. These dictionaries \eqref{eq:dict_represent}
can be interpreted as splines approximation with free knots with selected
dictionary and we will study the approximation results when $\mathbb{D}$ is
comprised of sigmoidal functions and ReLU.
...
\begin{definition}[Covering numbers and entropy]
    ...
    Let $(\mcal{F}, \norm{\cdot}_{\mcal{F}})$ be a subset of a normed space with
    norm $d$. The \textit{covering numbers} $N(\epsilon, \mcal{F},
    \norm{\cdot}_{\mcal{F}})$ is the minimal number of balls $\{g: \norm{f -
    g}_{\mcal{F}} < \epsilon\}$ of radius $\epsilon$ required to cover the set
    $\mcal{F}$. The \textit{entropy} $\log N(\epsilon, \mcal{F},
...
dictionary in a Hilbert space $\mcal{H}$ achieves a error rate of
...
made arbitrarily small when $n$ is allowed grow.
...
Firstly, we define the spectral conditions and the semi/norm\footnote{
...
The index or coefficients $s \in \Nat$ is referred as the smoothness index and
it is easy to conjecture that as $s$ increases, functions in $\bspace{\mcal{F},
s}$ are becoming ``smoother''. Therefore the \textit{size} of the function
spaces of higher smoothness index $s$ is expected to ``shrink'' which might
suggests better approximation error rate. In the coming sections, we will
...
\begin{definition}[Barron norm] For function $f$ that admits the integral
...
dimensionality $d$ with some restriction on the smoothness of the functions. The
result is extended to ReLU activation function. In other words, any functions in
that class can be approximated with a 2NN at an error rate of $\bigO(n^{-1}
\cdot C)$ where $n$ is the number of nodes in the the single hidden layer and
$C$ is a constant dependent \textit{only} the smoothness of the target function.
Even the convergence rate itself is independent of dimension (i.e. the
dimensionality $d$ of input vector $x\in\R^d$), the constant $C$ could be
dimension-dependent as the Fourier transform is used here.
...
\begin{proof}
    ...
    constant $C_G$. Then for every $N \geq 1$, and every constant $C' > C_g^2 -
...
improved to $\bigO(n^{\frac{1}{2} - \frac{1}{q\cdot d}})$ in $\lp{p}(\Omega)$, $q <
\infty$.
...
\begin{remark}
    ...
    It has been discussed in earlier section
    that we have the equivalence of Barron norm and hence Barron spaces for $1
    \leq p\leq +\infty$ (see \eqref{eq:barron_fouier_int}) with ReLU as
    activation function.
...
In this section, we will introduce the improved approximation error rates. The
core of the idea is to obtain the entropy number of the convex hull of the
dictionary $\mathbb{D}$. Maurey's Theorem \eqref{thm:maurey} provides a bound
for any dictionary in a Hilbert space. The improvement was first done
by~\cite{makovozRandomApproximantsNeural1996}. Subsequently, improvements were
stated where squared ReLU are used as the activation functions
\cite{klusowskiApproximationCombinationsReLU2018}. The first was possible via
the selection of Heaviside function and an better rate of $\bigO(n^{-1/2 -
1/q\cdot d})$ was obtained in Section \ref{subsec:improved_heaviside}. In the
latter, the \textit{inner parameters} $b\in\R^d$ in
\eqref{eq:improved_fourier_dict} are restricted to ensure the compactness. This
...
Previously, the smoothness of a function $f$ is expressed through its
\textit{first} Fourier representation and controlled via the spectral condition
$v_{f,s}$ \eqref{eq:spectral_condition}. In particular, how ``oscillating'' or
...
bounding the inner parameter $b\in\R^d$ in \eqref{eq:improved_fourier_dict}.
...
This implies we can write the Barron spaces $\bspace{\mcal{F}, s}$ in a similar
manner to \eqref{eq:dict_sigma}. We define a set of finite linear combination of
elements from $\mathbb{D}_{\mcal{F},s}$ in which $\abs{a_j}$ is bounded in
$\lp{1}$
\begin{equation}
