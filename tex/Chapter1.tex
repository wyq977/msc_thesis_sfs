...
\begin{proof}
    ...
    \begin{equation}
        \phi(A) = \mu(\{x \in I_d: b_o\tr x \in A\}) \quad \forall A \in \R.
    \end{equation}
    ...
    \begin{align}
        ...
        &= 0 - 0 = 0
    \end{align}
    thus $\phi(A) = 0$ for all Borel sets $A \in \R$.

    % Show a bounded linear functional is zero on such measure
    \textbf{Step 2} \textit{(Define a linear functional $L$ and show $L = 0$ on
    with $\mu$)}: Let $L: L^{\infty}(\R) \to \R$ be a bounded linear functional
    and $L^{\infty}(\R)$ denotes the space of all measurable bounded functions.
    For a function $h \in L^{\infty}(\R)$, we define $L$:
    \begin{equation}
        L(h) = \int_{I_d} h(b\tr)\,d\mu(x)
    \end{equation}
