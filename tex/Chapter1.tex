...
the \textit{spectral norm} by \cite{barronUniversalApproximationBounds1993} and
the \textit{Barron norm} by \cite{eBarronSpaceFlowinduced2021}. Despite the term
...
finite Fourier moment can be approximated with superpositions of sigmoidal
functions at a rate independent of the dimensionality $d$ 
(\tonote{where does the d come from}). In other words, any 
function in said class can be approximated with a two-layer neural network
at a error rate $\bigO(m^{-1} \cdot C)$ where $m$ is the number of nodes in the
the single hidden layer and $C$ is some constant dependent only
on the smoothness of the target function.
...
\begin{theorem}\cite[Theorem~1]{barronUniversalApproximationBounds1993}\label{thm:barron_1993_1}
    For every function $f$ in $\Gamma_C(X, x_0)$, every sigmoidal function $\phi$,
    every probability measure $\mu$, and every $n \geq 1$, there exists a linear
    combination $f_n$ of sigmoidal functions in the form \TOCITE(2NN), such that
    \begin{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \abs{\omega}_{X, x_0} 
        = \sup_{x\in X}\abs{\spr{\omega}{x-x_0}} 
        \leq \sup_{x\in X}\abs{\spr{\omega}{x-x_1}} 
            + \abs{\spr{\omega}{x_0-x_1}} 
        \leq 2\abs{\omega}_{X, x_1}
    \end{equation}

    thus we have $\int_{\mathbb{R}^d} \abs{\omega}_{X, x_1} \cdot
    ...
    Note that by polar decomposition we have $F(\omega) = e^{i\theta(\omega)} \cdot
    ...
    Similarly, we can construct $g_{k,-}(z) = \sum_{i=1}^{k-1} (g(-p_i) -
        g(-p_{i-1})) \cdot \stepfunction{z\leq -p_i}$, which results in a sequence of
    piecewise step function on $[-1, 1]$ uniformly close to $g(z)$. We have
    $g(z) = g_{k,+}+g_{k,-}$, a linear combination of step functions (or
    heaviside functions) and the sum of the coefficients is bounded by $2C$ (The
    ...
    the sequence converges to step functions pointwise (except at points where $\alpha x -
        t = 0$). If we introduce a measure $\mu$ that has zero measure at those
    points, the previous statement on $G_{cos} \subset G^{\mu}_{step}$ still holds
    on $\{\abs{t} \leq 1: \alpha x - t \not=0\}$ give a particular $\alpha$. We
    subsequently have convergence in $L_2(\mu, X)$ by the dominated convergence
    ...
    It has been shown above that function $f(x) - f(0)$ is in the closure of the convex
...
\begin{theorem}\label{thm:appr_f2}
    ...
    \begin{equation}
        ...
        - \mathbf{x} \cdot \nabla f(0) 
        - f(0)
        - f_m}_2^2 \leq \frac{16 v_{f, 2}^2}{m}
    \end{equation}
\end{theorem}

\textbf{What is the diff between the above and below?}

Below the norm seems to be the $L_{\infty}$ norm where the above the the $L_2$.
...
\begin{theorem}\label{thm:appr_f3} Consider the class of function on
    ...
    with $\norm{b_i}_2 = 1$ and $\abs{c_i}\leq 1$ such that
    \begin{equation}
        \norm{f 
        - \mathbf{x}^T H_f(0) \mathbf{x}/2 
        - \mathbf{x} \cdot \nabla f(0) 
        - f(0) 
        - f_m}_2^2 \leq \frac{16 v_{f, 3}^2}{m}
    \end{equation}
