...
networks} and subsequently concentrate on the special case of fully connected
networks. Although most targeted applications are utilizing other architecture,
e.g. recurrent neural network \TOCITE, fully connected feed-forward neural
networks allows researchers to connect the trade-off between model complexity
and approximation efficiency conveniently.
...
of vertices \footnotes{
    Nodes, neurons are used interchanably for the vertices and we refer to the 
    vertices as nodes.
}
...
\begin{itemize}
    \item A activation function $\sigma$ is associated with each $v \in \mathcal{V} \setminus \mathcal{I}$ 
    \item A scalar $w$ is associated with each $e \in \mathcal{E}$
\end{itemize}


The number of trainable parameters are determined by the weights and the bias of
$\mathcal{G}$. 
