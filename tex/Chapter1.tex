...
\begin{proof}
    Let $\sigma: \R \to \R$ be a sigmoidal function and $\mu \in M(I_d)$. Assume
    that the $\sigma$ is discriminatory with measure $\mu$ as in Def.
    \ref{def:dis_func} we need to show that the measure $\mu = 0$.

    Fixed a $b_0 \in \R^d\setminus\{0\}$. Note that we have
    \begin{equation}
        \label{eq:uat_lebesgue_conv}
        \int_{I_d} \sigma(\lambda b\tr x + c) \,d\mu(x) = 0 \quad \forall \lambda, c \in \R
    \end{equation}

    For any $b, \varphi \in \R$, let us define function $\gamma: I_d \to \R$
    \begin{equation}
        \gamma(x) = 
        \begin{cases}
        ...
        \end{cases}
    \end{equation}
    where $\sigma_{\lambda}(x)$ is set to $\sigma(\lambda(b\tr x + c) + \varphi)$.
    ...
    We denote the hyperplane by $\Pi_{b,c}$:
    \begin{equation}
        \Pi_{b,c} = \{x\in\R^d \mid b\tr x + c = 0\}
    \end{equation}
    and let $H_{b_0, c}$ be the half-space defined as:
    \begin{equation}
        H_{y_0, c} = \{x\in\R^d \mid b\tr x + c > 0\}.
    \end{equation}
